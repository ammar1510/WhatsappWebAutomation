...
while (len(number) != 0):
    try:
        ...
        # time.sleep(2)
        # k = name[0]
        url="https://web.whatsapp.com/send?phone=91"+str(num)
        browser.get(url)
        time.sleep(10)
        # if f==0:
        #     wait.until(EC.visibilityOfElementLocated(By.XPath('//*[@id="action-button"]')))
        #     chat_btn=browser.find_element_by_xpath('//*[@id="action-button"]')
        #     chat_btn.click()
        #     wait.until(EC.visibilityOfElementLocated(By.XPath('//*[@id="fallback_block"]/div/div/h4[2]/a')))
        #     whatsapp_web_btn = browser.find_element_by_xpath('//*[@id="fallback_block"]/div/div/h4[2]/a')
        #     whatsapp_web_btn.click()
        #     f = 1
       # search_box = browser.find_element_by_xpath('//*[@id="side"]/div[1]/div/label/div/div[2]')
        # search_box.clear()
        # time.sleep(2)
        ...
        try:
            ...
            time.sleep(10+0.2*randint(1, 10))
        except:
            print('no message found!')
        try:
            #         ----------send file--------------
