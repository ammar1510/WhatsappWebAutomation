...
while (len(number) != 0):
    try:
        ...
        c = 1.8
        # if whatsapp contact directory is exhaustive / search latency is higher uncomment line 121
        # c = 5
        time.sleep(c+0.2*randint(1, 10))
        # k = name[0]
        # text="z-index: 2; transition: none 0s ease 0s; height: 72px; transform: translateY(72px);"
        # browser.find_element_by_xpath("(//*[contains(text(), '" + text + "')] | //*[@value='" + text + "'])")

        userID = browser.execute_script("return Array.from(document.getElementsByClassName('lhggkp7q ln8gz9je rx9719la')).filter(element => element.getAttribute('style').includes('translateY(72px)'))[0]")
        userID.click()
        time.sleep(2)
        # search_box = browser.find_element_by_xpath('//*[@id="side"]/div[1]/div/label/div/div[2]')
        # search_box.clear()
